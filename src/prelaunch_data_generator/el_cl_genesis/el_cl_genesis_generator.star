...
def generate_el_cl_genesis_data(
    ...
    genesis = plan.run_sh(
        ...
        run="cp /opt/values.env /config/values.env && ./entrypoint.sh all && mkdir /network-configs && mv /data/custom_config_data/* /network-configs/ && mv /config/values.env /network-configs/",
        image=image,
        files=files,
        store=[
